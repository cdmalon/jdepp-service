...
//  $Id: typedef.h 827 2012-05-10 14:12:37Z ynaga $
...
namespace ny {
#ifdef USE_DARTS
  static const std::string TRIE_SUFFIX = "da";
#endif
#ifdef USE_DARTS_CLONE
  static const std::string TRIE_SUFFIX = "dan";
#endif
#ifdef USE_CEDAR
  static const std::string TRIE_SUFFIX = "cda";
#endif
  // type alias
...
#ifdef USE_CEDAR
  typedef cedar::da <int, -1, -2, 32>  trie;
#else
  ...
  struct set { typedef std::tr1::unordered_set <_Key> type; };
#else
  template <typename _Key, typename _Tp>
  struct map { typedef std::map <_Key, _Tp> type; };
  template <typename _Key>
  struct set { typedef std::set <_Key> type; };
#endif
  template <typename T>
  ...
  public:
    ...
    size_t len;
    U*     cont;
    TrieKeyBase () : key (0), len (0), cont (0) {}
    TrieKeyBase (T* k, U* c, size_t l = 0, size_t nl = 1)
      : key (NULL), len (l ? l : length (k)), cont (NULL) {
      key = new T[len + 1];
      key[len] = '\0';
